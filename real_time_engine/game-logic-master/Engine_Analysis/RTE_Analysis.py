...
def list_files(rootdir):
    ...
    'Accuracy'+','+'Specificity'+','+'Sensitivity'+','+'Precesion'+','+'F2Score')
    fileCSV.write("\n")
    ...
    for item in list:
        ...
        if (int(item[0])+int(item[1])) ==0:
            ...
            precesion = (int(item[0]))/(int(item[0])+int(item[1]))
      
      
       #f2 score
       #division by error issue, needs to be looked into more. 
        if (float(precesion)+float(sensitivity)) == 0.0 or precesion*sensitivity==0:
            f2score=0
        else:
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>",precesion+sensitivity,precesion*sensitivity)
            f2score=2 * ((float(precesion)*float(sensitivity))/precesion+sensitivity)
        ...
        fileCSV.write(str(precesion)+",")
        fileCSV.write(str(f2score))
        fileCSV.write("\n")
        i+=1
